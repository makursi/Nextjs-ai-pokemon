...
> **关于语言**：本文档用中文书写，方便本人阅读；代码、注释、ADR 与 commit message 仍保持英文（那是仓库的既有约定）。**注意**：站点目前的文案仍是英文，改中文的方向已经定了但**尚未实施**，具体见「待办」第 1 条。
...
行高由 token 控制（`--mantine-line-height`，当前 `1.6`）。**中文文案上线时需要调整**：CJK 需要更松的行距，见「待办」第 1 条。
...
封面在 `ToolMeta` 里是**可选的**，所以不会有 Tool 因为等图而不能上线。它的 4:3 图框**无论文件在不在都预留**，这样图到位那天不会有任何布局跳动。图没来时框是空的：**不放任何替代图形**——伪造一个 logo、或用 div 拼一张假截图，正是协议明令禁止的。`registry.test.ts` 会校验声明出来的 `cover` 是同源图片路径。
...
1. **全站主体文案改成中文**（方向已定，尚未实施）。要一起处理的连带项：
   - `<html lang="en">` 改成 `lang="zh-CN"`，否则读屏与搜索引擎都会按英文处理；
   - **字体栈加 CJK 回退**：Geist 没有中文字形，现在中文会落到各系统的默认中文字体。要么接受系统回退（PingFang SC / 微软雅黑 / Noto Sans CJK），要么用 `next/font` 自托管一套中文字体（构建期下载、自托管，不违反 CSP，代价是体积）；
   - **行高**从 `1.6` 提到约 `1.7`，CJK 需要更松的行距；
   - 标点用全角，并且**避免中文的 `——`**：理由和英文的破折号禁令一样，中文里的 `——` 同样是 AI 味的重灾区，改用逗号、分号或冒号；
   - 格式缩写（PNG / JPEG / WebP / AVIF / BMP）与品牌名（Toolbox）**保留拉丁写法**；
   - `<title>` 模板的分隔符从 `·` 改成 `|`（中文界面更常见的写法，也符合"中点要节约使用"的规则）。
2. **图标，用 Iconify 实现**（方向已定，尚未实施）。已核实：`@iconify/tailwind4`（1.2.3）是 Tailwind v4 的集成方式，`@iconify-json/ph`（1.2.2）是 Phosphor 图标集——Phosphor 正是设计语言规定的图标集。**约束**：图标绝不能在运行时去取，因为 `@iconify/react` 的默认行为是请求 `api.iconify.design`，那会被 CSP 拦掉；必须走构建期方案。**接线前要先查 Iconify 官方文档确认用法，不凭记忆。** 第一批要替换的是折叠用的 `+` / `−` 和卡片上的 `→`。
...
3. **logo 与封面图**：两张图到位后，卡片上的空图框才有内容。
4. **在浏览器里验证**：深色模式**从未被渲染过**，hover 与入场动效也**没有被看过**，Lighthouse **没跑过**。本文档其余内容都是对着构建产物验证的，不是用眼睛。
